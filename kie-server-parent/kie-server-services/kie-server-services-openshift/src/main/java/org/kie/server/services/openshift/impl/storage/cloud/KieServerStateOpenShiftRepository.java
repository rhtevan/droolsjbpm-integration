...
public class KieServerStateOpenShiftRepository extends KieServerStateCloudRepository implements KieServerOpenShiftUtils {
    ...
    public synchronized void create(@NotNull KieServerState kieServerState) {
        ...
        processKieServerStateByOpenShift(client -> {
            if (getKieServerCM(serverId, client).isPresent()) {
    ...
    public boolean exists(String id) {
        return processKieServerStateByOpenShift(client -> getKieServerCM(id, client).isPresent());
    }
    ...
    public KieServerState delete(String id) {
        ...
        boolean isUnsupported = processKieServerStateByOpenShift(client -> {
            if (getKieServerDC(id, client).isPresent()) {
                return true;
            } else {
    ...
    public synchronized void store(@NotNull String serverId, @NotNull KieServerState kieServerState) {
        ...
        processKieServerStateByOpenShift(client -> {
            Optional<DeploymentConfig> dcOpt = getKieServerDC(serverId, client);
            Optional<ConfigMap> cmOpt = getKieServerCM(serverId, client);
    ...
    public KieServerState load(@NotNull String serverId) {
        KieServerState kieServerState = processKieServerStateByOpenShift(client -> {
            ...
            ConfigMap cm = cmOpt.orElseGet(() -> {
                    ...
                    return createNewKieServerStateCM(initkieServerState, serverId, client);
                });
    ...
    public Optional<String> getAppName(String serverId, OpenShiftClient client) {
        Optional<String> appNameFromDC = getKieServerDC(serverId, client)
    ...
    public ConfigMap createNewKieServerStateCM(KieServerState kieServerState, String serverId, OpenShiftClient client) {
        String stateXML = xs.toXML(kieServerState);
        Map<String, String> labels = new HashMap<>();
        Optional<String> appName = getAppName(serverId, client);
        Optional<DeploymentConfig> dcOpt = getKieServerDC(serverId, client);
    ...
    private boolean isKieServerGlobalDiscoveryEnabled() {
        return Boolean.parseBoolean(System.getProperty(
            CFG_KIE_CONTROLLER_OCP_GLOBAL_DISCOVERY_ENABLED,
